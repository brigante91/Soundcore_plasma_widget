...
pub async fn handle(matches: &ArgMatches) -> anyhow::Result<()> {
    ...
    // Get MAC address from CLI or fall back to config default
    let mac_address = matches
        .get_one::<MacAddr6>("mac-address")
        .copied()
        .or(config.default_device)
        .ok_or_else(|| anyhow!(
            "No device specified. Use --mac-address or set default_device in config file."
        ))?;
